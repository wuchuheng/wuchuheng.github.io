...
在写`react`是需要组件和组件之间的通信，本来想用`redux`，但自己忘了怎么用了，翻文档案卷又太累，感觉很麻烦，所以就只能自己动手写一个，才能维持下生活这样子。
...
* 已经打包成为`npm`库了, [@wuchuheng/rxjs](https://www.npmjs.com/package/@wuchuheng/rxjs), `npm i @wuchuheng/rxjs`
